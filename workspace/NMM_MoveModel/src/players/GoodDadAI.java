...
public class GoodDadAI extends AbstractPlayer {
	...
	private double opponentFitness = 10.0;
	...
	private void recalculatePlayerFitness(){
		...
		if(opponentsPlay != null){
			...
			double ourScore = opponentsPlay.rewards[getPlayerID() - 1];
				...
				if(opponentsPlay.rewards[opponentID - 1] > opponentsPlay.rewards[getPlayerID() -1]){
					opponentFitness += 1.0;
				}else if(opponentsPlay.rewards[opponentID - 1] < opponentsPlay.rewards[getPlayerID() -1]){
					opponentFitness -= 1.0;
				}
			}
		}else{
