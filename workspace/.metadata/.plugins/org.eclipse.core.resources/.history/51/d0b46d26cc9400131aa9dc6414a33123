...
public class MCTSMoveEngine {
	...
	public List<Move> getAvailableMoves(char action, String state, char token) {
		ArrayList<Move> moves = new ArrayList<Move>();
		setState(state);
		
		switch (action) {
		case 'P': 
			moves = getAllPlacements(token);
			break;
		case 'M':
			moves = getAllMovements(token);
			break;
		case 'R':
			moves = getAllRemovals(token);
			break;
	...
	private ArrayList<Move> getAllPlacements(char token) {
	...
	private ArrayList<Move> getAllMovements(char token) {
	...
	private ArrayList<Move> getAllRemovals(char token) {
		...
		for (int i = 0; i < stateArray.length; i++) {
			Move m = new Move(state, 'R', token);
			if(stateArray[i] != 'N' || stateArray[i] != token){
				if(!partOfMill(i)){
					m.setRemoval(i);
					moves.add(m);
				}else if(partOfMill(i) &&  onlyAvailable(token, i)){
					m.setRemoval(i);
					moves.add(m);
				}
			}
