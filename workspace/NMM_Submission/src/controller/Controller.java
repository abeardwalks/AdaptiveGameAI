...
public class Controller {
	...
	private void start() {
		...
		thread = new Thread(new Runnable() {
			...
			@Override
			public void run() {					//sets the thread up to call the play method when started.
				play();
			}
		});
	...
	private void executeMove(Player player, BoardFacadeInterface model){
		...
		while(!played){
			if(model.getNextAction() == 'P' && !played){		//placement action?
			...
			if(model.getNextAction() == 'M' && !played){		//movement action?
			...
			if(model.getNextAction() == 'R' && !played){		//removal action?
